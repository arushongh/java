...
Think of streams like a **factory conveyor belt** 🏭

```
Stream data  ──▶  Collector  ──▶  Final Result
```
...
✔ Perform grouping, partitioning, aggregation easily

📌 **Collectors are used ONLY with `collect()`** (terminal operation)
...
## 🎬 Step-by-Step Walkthrough (What to Observe)

### 🟢 1. `toList()` – Filtering & Collecting

📌 **Question**: What happens to elements that don’t match the condition?

➡️ They are **dropped**, not stored anywhere.
...
### 🟢 2. `toSet()` – Removing Duplicates

📌 **Key idea**: Sets **never allow duplicates**
...
### 🟢 3. `joining()` – Stream → String

📌 Converts multiple elements into **one single String**
...
### 🟢 5. `groupingBy()` – Classification

📌 Think of it as **bucket sorting** 🪣
...
### 🟢 6. `partitioningBy()` – Boolean Split

📌 Only **two buckets**: `true` and `false`
...
### 🟢 7. `toMap()` with Merge Function

📌 Required when **duplicate keys exist**
...
## 🧠 One-Glance Collector Cheat Sheet
...
💡 `Collectors` only work with `collect()`
...
Instead of using ready‑made collectors like: - `Collectors.toList()` -
...
public class CustomCollectorExample {
    ...
    public static void main(String[] args) {
        ...
        Collector<Integer,double[],Double> avgCollector = Collector.of(
                ...
                // 1️⃣ Supplier → Creates container
                // double[0] = sum
                // double[1] = count
                () -> new double[]{0,0},

                // 2️⃣ Accumulator → Adds each element into container
                (arr,num)-> {
                    arr[0] += num;   // add to sum
                    arr[1]++;        // increase count
                },

                // 3️⃣ Combiner → Used in parallel streams
                // merges two partial results
                (j1,j2)-> new double[]{j1[0]+j2[0], j1[1]+j2[1]},

                // 4️⃣ Finisher → Converts container to final result
                // calculate average
                (x)-> x[0] / x[1]
        );
...
## How Collector.of Works (Simple Words)

| Part \| Meaning \|

\|-----\|------\| Supplier \| Creates an empty box to store data \| \|
Accumulator \| Puts each stream element into the box \| \| Combiner \|
Merges boxes (important for parallel streams) \| \| Finisher \| Converts
the box into final answer \|
