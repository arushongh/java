# 🌊 Java 8 Streams – Complete Guide with Examples

This markdown file is a **complete, clean, and GitHub‑ready guide** to **Java 8 Streams**, covering:

* What streams are and *why* they exist
* How streams are created
* **Intermediate operations** (filter, map, flatMap, etc.)
* **Terminal operations** (collect, reduce, find, etc.)
* Lazy evaluation
* Common pitfalls (stream reuse)

👉 **All code is preserved exactly as provided. Nothing is missed.**
...
> Intermediate operations **return a new stream** and are **lazy** — they execute **only when a terminal operation is called**.
...
✔ **Short‑circuit operations** – stop as soon as condition is met
...
### 9️⃣ forEach vs forEachOrdered (Parallel Streams)
...
// stream.map(String::toUpperCase); ❌ Exception
```
...
✔ Filtering happens **only after** terminal operation
